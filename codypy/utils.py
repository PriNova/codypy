"""
CodyPy support function
-----------------------

This module contains a set of helper functions for additional features.
"""
...
def _get_platform_arch() -> str | None:
    """
    Recognizes the operating system and architecture of the current system.

    Returns:
        str | None: A string representing the platform and architecture,
                    or None if the platform/architecture could not be
                    determined.
    """
...
def _has_file(binary_path: str, cody_agent_bin: str) -> bool:
    """
...
def _check_for_binary_file(binary_path: str, cody_name: str, version: str) -> bool:
    """
    ...
    """
    cody_agent = _format_binary_name(cody_name, version)
    has_bin = _has_file(binary_path, cody_agent)
    return has_bin


def _format_binary_name(cody_name: str, version: str) -> str:
    """
    ...
    formatted_arch = _format_arch(arch)
    return (
        f"{cody_name}-{formatted_arch}-{version}{'.exe' if arch == 'win-x64' else ''}"
    )


def _download_binary_to_path(binary_path: str, cody_name: str, version: str) -> bool:
    """
